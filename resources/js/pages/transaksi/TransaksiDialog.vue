  ...
  <div v-if="open" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4">
    <div class="bg-white rounded-2xl shadow-2xl p-4 sm:p-6 md:p-8 w-full max-w-lg transform transition-all duration-300 max-h-[90vh] overflow-y-auto">
      <div class="flex items-center justify-between mb-4 sm:mb-6">
        <h2 class="text-lg sm:text-xl md:text-2xl font-bold text-gray-800">{{ isEdit ? 'Edit Transaksi' : 'Tambah Transaksi Baru' }}</h2>
      ...
      <form @submit.prevent="handleSubmit" class="space-y-4 sm:space-y-6">
        ...
        <div class="space-y-2">
          ...
          <div class="relative">
            <select 
              ...
              v-model="form.brand" 
              required 
              class="w-full px-3 sm:px-4 py-2 sm:py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 appearance-none text-sm sm:text-base"
            >
          ...
          <div v-if="form.brand && selectedBrandData" class="mt-2 p-2 sm:p-3 bg-gray-50 rounded-lg flex items-center gap-2 sm:gap-3">
            <div v-if="selectedBrandData.logoPath" class="w-10 h-10 sm:w-12 sm:h-12 rounded-lg overflow-hidden bg-white border border-gray-200 flex items-center justify-center flex-shrink-0">
              <img 
            ...
            <div class="min-w-0 flex-1">
              <p class="font-medium text-gray-900 text-sm sm:text-base truncate">{{ selectedBrandData.namaBrand }}</p>
              <p class="text-xs sm:text-sm text-gray-500 truncate">{{ selectedBrandData.namaCV }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="space-y-2">
          ...
          <div class="relative">
            <span class="absolute left-3 sm:left-4 top-1/2 transform -translate-y-1/2 text-gray-500 font-medium text-sm sm:text-base">Rp</span>
            <input 
              type="number" 
              id="nominal" 
              v-model.number="form.nominal" 
              required 
              class="w-full pl-10 sm:pl-12 pr-3 sm:pr-4 py-2 sm:py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-200 text-sm sm:text-base"
              placeholder="0"
              min="0"
              step="1000"
            />
